"""
example using NewtonSystem solve 
"""
import numpy as np
from newton_system_iteration import NewtSysSolve
import matplotlib.pyplot as plt          # For plotting


def f(x):  # defines the function (remember this is a vector)
    x1 = x[0]
    x2 = x[1]
...
def Df(x):  # defines the Jacobian
    x1 = x[0]
    x2 = x[1]
    J11 = 2.0*np.exp(x1*x2)*x2 - 2.0 # partial derivate df1/dx1
    J12 = 2.0*np.exp(x1*x2)*x1 + 2.0 # partial derivate df1/dx2
    J21 = 5.0*x1**4 + x2**5 # partial derivate df2/dx1
    J22 = 5.0*x1*x2**4 - 2.0 # partial derivate df2/dx2
